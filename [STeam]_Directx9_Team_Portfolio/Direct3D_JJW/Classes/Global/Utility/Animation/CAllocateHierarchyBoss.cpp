#include "CAllocateHierarchyBoss.h"
#include "../Manager/CResourceManager.h"

CAllocateHierarchyBoss::CAllocateHierarchyBoss(const STParameters & parameters)
	:
	_parameters(parameters)
{
}

HRESULT CAllocateHierarchyBoss::CreateFrame(const char * name, LPD3DXFRAME * newFrame)
{
...
HRESULT CAllocateHierarchyBoss::CreateMeshContainer(const char * name,
	...
	LPD3DXMESHCONTAINER * newMeshContainer)
{
	...
	for (int i = 0; i < numMaterials; ++i) {
		...
		if (material.pTextureFilename != nullptr) {
			...
			std::string temp = material.pTextureFilename;
			std::string normalTextureName = "";

			if (temp.rfind('_') != std::string::npos) {
				size_t index = temp.rfind('_');
				normalTextureName = temp.substr(0, index) + "_norm.tga";
			}

			sprintf(textureFilepath, "%s/%s",
				_parameters.basePath.c_str(),
				normalTextureName.c_str());

			texture = GET_SPRITE_TEXTURE(textureFilepath);
		}
		meshContainer->normalTextureList.push_back(texture);
	}
	//}
...
HRESULT CAllocateHierarchyBoss::DestroyFrame(LPD3DXFRAME frameToFree)
{
...
HRESULT CAllocateHierarchyBoss::DestroyMeshContainer(LPD3DXMESHCONTAINER meshContainerToFree)
{
	return S_OK;
}
