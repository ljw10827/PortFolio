...
void CTeraUI::init(void)
{
	...
	if (_mapKind == EMapKind::mainMap){
		...
		m_pMiniMap->setPosition(D3DXVECTOR3(GET_WINDOW_SIZE().cx - m_miniMapSize.cx / 2 - 250, m_miniMapSize.cy / 2 + 200, 0.5f));
...
void CTeraUI::doDrawUI(void)
{
	...
	if (_mapKind == EMapKind::mainMap) {
		...
		for (auto iterator : _enemyList) {
			if (iterator.first->getState() != CMonster::EState::DEATH) {
				iterator.second->_isDie = false;
			}
			else if (iterator.first->getState() != CMonster::EState::DEATH) {
...
void CTeraUI::update(void)
{
	...
	if (_mapKind == EMapKind::mainMap) {
		...
		this->radarSetting(D3DXVECTOR3(20.0f, 0.0f, 20.0f), _boss);
	}
}
...
{
	_playerhp -= damage;
	_hpPercent -= (100 * damage) / 5000;
	//들어오는 데미지량을 총량의 체력에 비례해서 깍는다..

	//printf("hp : %d", _playerhp);

	//printf("%d\n", _hpGaugeFull->getTextureSize().cx);
...
void CTeraUI::radarSetting(const D3DXVECTOR3 & a_rtPosition, CSprite_MapUI * target)
{
	...
	if (_mapKind == EMapKind::mainMap) {
		...
		// 현재 마우스 위치 확인
		m_pRadarAngle += (stMousePosition.x - m_stPrevMousePosition.x) * 0.1f;
		m_pRadarBase->SetMapUIRotation(-m_pRadarAngle);
		target->SetMapUIRotation(-m_pRadarAngle);
